  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="order-info-card">
      <div class="order-header">
        ...
        <span class="order-title">销售订单明细 — {{ orderNo }}</span>
      </div>
    </el-card>
...
const getList = async () => {
  loading.value = true
  try {
    const res = await getDetailListByOrderId(orderId.value)
...
const handleSubmit = async () => {
  ...
  try {
    const payload = { ...form, orderId: orderId.value }
    if (payload.id) {
    ...
    // Refresh product type options in case a new type was added
    loadProductTypes()
    getList()
  } finally {
...
const handleExport = async () => {
  exportToCsv('销售订单明细导出', detailList.value, [
    { label: '产品规格', key: 'spec' },
...
onMounted(async () => {
  // Load order no from the passed query param or resolve it
  const no = route.query.orderNo as string
  orderNo.value = no || `订单 #${orderId.value}`
