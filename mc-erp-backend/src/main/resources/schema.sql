...
CREATE TABLE IF NOT EXISTS `biz_purchase_order` (
  ...
  `create_id` bigint(20) DEFAULT NULL COMMENT '创建人ID',
  `currency` varchar(10) DEFAULT 'RMB' COMMENT '币种: RMB / USD',
  `total_amount` decimal(15,2) NOT NULL DEFAULT '0.00' COMMENT '合同总金额（由明细自动计算）',
...
CREATE TABLE IF NOT EXISTS `biz_purchase_order_detail` (
  ...
  `price_total` decimal(15,2) DEFAULT NULL COMMENT '合同金额小计',
  `settlement_amount` decimal(15,2) DEFAULT NULL COMMENT '结算金额小计',
